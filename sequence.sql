/************************************************************************/
시퀀스 (Sequence)
PK로 사용하거나 순차적인 데이터를 표현하기 위해 사용하는 숫자 값
...
select COUNT(*)+1 from seq_test; --다음 들어갈 no값
...
--서브쿼리 활용 (마지막 NO 값, MAX)
select MAX(no) from seq_test;   --현재 no 최대값
select MAX(no)+1 from seq_test; --다음에 사용할 no값
select NVL(MAX(no)+1, 1) from seq_test; --다음에 사용할 no값
select NVL(MAX(no), 0) from seq_test; --다음에 사용할 no값
