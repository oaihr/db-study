...
MERGE INTO T_COMPANY CP
USING T_SHOE S
ON (cp.work_date = S.work_date) --날짜 하나로는 병합기준으로 사용 불가능!
                                --동일 날짜가 겹쳐서 진행이 안됨
WHEN MATCHED THEN              
    UPDATE SET cp.sales_income = S.sales_income
WHEN NOT MATCHED THEN 
    INSERT VALUES (S.work_date, S.store_code, S.sales_income);    
...
MERGE INTO T_COMPANY CP
USING T_SHOE S
ON (cp.work_date = S.work_date AND cp.store_code = S.store_code) 
WHEN MATCHED THEN              
    UPDATE SET cp.sales_income = S.sales_income
WHEN NOT MATCHED THEN 
    INSERT VALUES (S.work_date, S.store_code, S.sales_income); 
...
WHEN MATCHED THEN              
    UPDATE SET cp.sales_income = S.sales_income
WHEN NOT MATCHED THEN 
    INSERT VALUES (S.work_date, S.store_code, S.sales_income, 'S');     
